...
TITLE iotracah Service ${project.version}
...
set EXECUTABLE=%IOT_HOME%\bin\iotracah-service-x64.exe
set SERVICE_ID=iotracah-service-x64
set ARCH=64-bit
goto checkExe

:x86
set EXECUTABLE=%IOT_HOME%\bin\iotracah-service-x86.exe
set SERVICE_ID=iotracah-service-x86
set ARCH=32-bit

:checkExe
if EXIST "%EXECUTABLE%" goto okExe
echo iotracah-service-(x86|x64).exe was not found...

:okExe
set IOT_VERSION=${project.version}

if "%LOG_DIR%" == "" set LOG_DIR=%IOT_HOME%\logs
...
:doManagment
set EXECUTABLE_MGR=%IOT_HOME%\bin\iotracah-service-mgr.exe
...
:foundJVM
if "%IOT_MIN_MEM%" == "" set IOT_MIN_MEM=${packaging.iotracah.heap.min}
if "%IOT_MAX_MEM%" == "" set IOT_MAX_MEM=${packaging.iotracah.heap.max}

if NOT "%IOT_HEAP_SIZE%" == "" set IOT_MIN_MEM=%IOT_HEAP_SIZE%
if NOT "%IOT_HEAP_SIZE%" == "" set IOT_MAX_MEM=%IOT_HEAP_SIZE%

call:convertxm %IOT_MIN_MEM% JVM_XMS
call:convertxm %IOT_MAX_MEM% JVM_XMX
...
CALL "%IOT_HOME%\bin\iotracah.in.bat"

rem thread stack size
set JVM_SS=256

if "%DATA_DIR%" == "" set DATA_DIR=%IOT_HOME%\data

if "%CONF_DIR%" == "" set CONF_DIR=%IOT_HOME%\config

set IOT_PARAMS=-Diotracah;-Diotracah.path.home="%IOT_HOME%";-Diotracah.default.path.home="%IOT_HOME%";-Diotracah.default.path.logs="%LOG_DIR%";-Diotracah.default.path.data="%DATA_DIR%";-Diotracah.default.path.conf="%CONF_DIR%"

set JVM_OPTS=%JAVA_OPTS: =;%

if not "%IOT_JAVA_OPTS%" == "" set JVM_IOT_JAVA_OPTS=%IOT_JAVA_OPTS: =#%
if not "%IOT_JAVA_OPTS%" == "" set JVM_OPTS=%JVM_OPTS%;%JVM_IOT_JAVA_OPTS%

if "%IOT_START_TYPE%" == "" set IOT_START_TYPE=manual
if "%IOT_STOP_TIMEOUT%" == "" set IOT_STOP_TIMEOUT=0

"%EXECUTABLE%" //IS//%SERVICE_ID% --Startup %IOT_START_TYPE% --StopTimeout %IOT_STOP_TIMEOUT% --StartClass org.elasticsearch.bootstrap.Elasticsearch --StopClass org.elasticsearch.bootstrap.Elasticsearch --StartMethod main --StopMethod close --Classpath "%IOT_CLASSPATH%" --JvmSs %JVM_SS% --JvmMs %JVM_XMS% --JvmMx %JVM_XMX% --JvmOptions %JVM_OPTS% ++JvmOptions %IOT_PARAMS% %LOG_OPTS% --PidFile "%SERVICE_ID%.pid" --DisplayName "Elasticsearch %IOT_VERSION% (%SERVICE_ID%)" --Description "Elasticsearch %IOT_VERSION% Windows Service - http://elasticsearch.org" --Jvm "%JVM_DLL%" --StartMode jvm --StopMode jvm --StartPath "%IOT_HOME%" ++StartParams start
