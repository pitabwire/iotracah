...
public class IOTMqttTransformerImpl implements IOTMqttTransformer{
    ...
    public MqttMessage toServerMessage(IOTMessage internalMessage) {
        ...
        if(internalMessage instanceof PublishMessage){
        ...
        }else if(internalMessage instanceof AcknowledgeMessage){
        ...
        }else if(internalMessage instanceof PublishReceivedMessage){
            ...
            MqttMessageIdVariableHeader msgIdVariableHeader = MqttMessageIdVariableHeader.from(pubrec.getMessageId().intValue());
            return MqttMessageFactory.newMessage(recFixedHeader, msgIdVariableHeader, null);


        }else if(internalMessage instanceof ReleaseMessage){
            ...
            MqttMessageIdVariableHeader msgIdVariableHeader = MqttMessageIdVariableHeader.from(pubrel.getMessageId().intValue());
            return MqttMessageFactory.newMessage(relFixedHeader, msgIdVariableHeader, null);


        }else if(internalMessage instanceof DestroyMessage){
            ...
            MqttMessageIdVariableHeader msgIdVariableHeader = MqttMessageIdVariableHeader.from(destroyMessage.getMessageId().intValue());
            return MqttMessageFactory.newMessage(compFixedHeader, msgIdVariableHeader, null);

        }else if(internalMessage instanceof Ping){

            Ping ping = (Ping) internalMessage;
            //We need to generate a PINGRESP message to respond to a PINGREQ.
            MqttFixedHeader recFixedHeader = new MqttFixedHeader(MqttMessageType.PINGRESP, ping.isDup(), MqttQoS.valueOf(ping.getQos()), ping.isRetain(), 0);
            return MqttMessageFactory.newMessage(recFixedHeader, null, null);

        }else if(internalMessage instanceof ConnectAcknowledgeMessage){
        ...
        }else if(internalMessage instanceof SubscribeAcknowledgeMessage){
        ...
        }else if(internalMessage instanceof UnSubscribeAcknowledgeMessage){

            UnSubscribeAcknowledgeMessage unSubAckMsg = (UnSubscribeAcknowledgeMessage) internalMessage;
            MqttFixedHeader respFixedHeader = new MqttFixedHeader(MqttMessageType.UNSUBACK, unSubAckMsg.isDup(), MqttQoS.valueOf( unSubAckMsg.getQos()), unSubAckMsg.isRetain(), 0);
